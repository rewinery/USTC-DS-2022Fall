...
void getCode(HTNode ht[], int c, unsigned* code1, int* clen1) {
	...
	while (parent >= 0) {
		code = code >> 1;
		ht[parent].lchild == c ? code : (code = code | mask); //如果是右孩子，那么最高位设置为1，否则就是0 
		clen++;
		c = parent;
		parent = ht[c].parent;
	}
...
int main() {
	...
	// 压缩一个图片
	zip("HollowKnight.png","HollowKnight.png.myzip");
	unzip("HollowKnight.png.myzip","myout_pic.png");
